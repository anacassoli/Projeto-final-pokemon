...
function createCardHTML(data) {
    ...
    const color = typeColors[mainType] || "#57595cff";

    // TIPOS EM HTML (agora CORRIGIDO)
    const typesHtml = data.types
        .map(t => {
            const tName = t.type.name;
            const tColor = typeColors[tName] || "#57595cff";
            return `
...
(async function init() {
  ...
  await fetchAndCreateCard("tyranitar");
  await fetchAndCreateCard("sableye");
  await fetchAndCreateCard("sneasel");
  await fetchAndCreateCard("murkrow");
  await fetchAndCreateCard("houndoom");
  await fetchAndCreateCard("poochyena");
  await fetchAndCreateCard("sharpedo");
  await fetchAndCreateCard("absol");
  await fetchAndCreateCard("stunky");
  await fetchAndCreateCard("spiritomb");
  await fetchAndCreateCard("darkrai");
